...
def ingredient_new():
    ...
    if form.validate_on_submit():
        ...
        try:
            db.session.commit()
            flash(
                f"You've added the ingredient '{ingredient}', ${ingredient.unit_price_dollars}.{ingredient.unit_price_cents} per {ingredient.unit}!"
            )
        except:
            db.session.rollback()
            flash(
                f"Ingredient '{form.name.data}' already exists! Do you want to edit it?"
            )
            return render_template(
...
def ingredient_edit(ingredient_id):
    ...
    if form.validate_on_submit():
        ...
        try:
            db.session.commit()
            flash(
                f"You've edited ingredient '{ingredient}', ${ingredient.unit_price_dollars}.{ingredient.unit_price_cents} per {ingredient.unit}!"
            )
            return redirect(url_for("inventory.ingredients"))
        except:
            (f"Ingredient '{form.name.data}' already exists! Do you want to edit it?")
...
def ingredient_delete(ingredient_id):
    ...
    if request.method == "POST":
        if "yes" in request.form:
            ...
            db.session.commit()
            flash(f"You've deleted ingredient '{ingredient}'.")
        return redirect(url_for("inventory.ingredients"))
...
def menu_item_new():
    ...
    if form.validate_on_submit():
        ...
        try:
            db.session.commit()
            flash(
                f"You've added the menu item '{menu_item}', price: ${menu_item.price_dollars}.{menu_item.price_cents}."
            )
        except:
            db.session.rollback()
            flash(f"Menu item {form.title.data} already exists!")
            return render_template(
...
def menu_item_edit(menu_item_id):
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.commit()
            flash(
                f"You've edited menu_item '{menu_item}, price: ${menu_item.price_dollars}.{menu_item.price_cents}."
            )
            return redirect(url_for("inventory.menu_items"))
        except:
            flash(f"The menu item '{form.title.data}' already exists!")
            db.session.rollback()
    return render_template(
...
def menu_item_delete(menu_item_id):
    ...
    if request.method == "POST":
        if "yes" in request.form:
            flash(f"You've deleted menu_item '{menu_item}'.")
...
def recipe_requirement_new():
    ...
    if form.validate_on_submit():
        ...
        if not RecipeRequirement.query.filter(
            ...
            db.session.commit()
            flash(
                f"You've added the recipe requirement for {menu_item} ({recipe_requirement})."
            )
            return redirect(url_for("inventory.recipe_requirements"))
        flash(
            f"The recipe requirement '{ingredient} for {menu_item}' already exists! Do you want to edit it?"
        )
...
def recipe_requirement_edit(recipe_requirement_id):
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        flash(
            f"You've edited the recipe requirement for {recipe_requirement.menu_item} ({recipe_requirement})."
        )
        return redirect(url_for("inventory.recipe_requirements"))
...
def recipe_requirement_delete(recipe_requirement_id):
    ...
    if request.method == "POST":
        if "yes" in request.form:
            flash(
                f"You've deleted a recipe_requirement for {recipe_requirement.menu_item}."
            )
...
def purchase_delete(purchase_id):
    ...
    if request.method == "POST":
        if "yes" in request.form:
            ...
            db.session.commit()
            flash(f"You've removed the purchase from the calculations.")
        return redirect(url_for("inventory.purchases"))
