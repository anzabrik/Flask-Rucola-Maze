...
- 'inventory', которое осуществляет все операции с ингредиентами, пунктами меню и покупками. У URL-адресов здесь нет префиксов, поскольку это приложение более важноеж
- 'auth', которое отвечает за авторизацию и аутентификацию пользователей. URL-адреса начинаются с 'auth'.

У каждого из суб-приложений отдельный файл с views ('inventory.py' и 'auth.py'), а также отдельная папка с шаблонами. Суб-приложения связаны через файл '__init__.py', в котором находится фабрика приложений. 
